...
namespace PxApi.UnitTests.ControllerTests
{
    ...
    public class HierarchyControllerTests
    {
        ...
        private Mock<ILogger<HierarchyController>> _mockLogger;
        ...
        public void SetUp()
        {
            ...
            _mockLogger = new Mock<ILogger<HierarchyController>>();
            _controller = new HierarchyController(_cachedDbConnector.Object, _mockLogger.Object)
            {
        ...
        public void GetHierarchy_DatabaseNotFound_ReturnsNotFound()
        {
            ...
            // Act
            ActionResult<Dictionary<Groupings, List<PxFileRef>>> result = _controller.GetHierarchy(databaseId);
        ...
        public void GetHierarchy_HierarchyNotFound_ReturnsNotFound()
        {
            ...
            _cachedDbConnector
                .Setup(x => x.TryGetDataBaseHierarchy(dbRef, out It.Ref<Dictionary<string, List<string>>?>.IsAny))
                .Returns(false);
        ...
        public void UpdateHierarchy_DatabaseNotFound_ReturnsNotFound()
        {
            ...
            // Act
            ActionResult result = _controller.UpdateHierarchy(databaseId, hierarchyToUpdate);
        ...
        public void UpdateHierarchy_ValidDatabase_CallsSetDataBaseHierarchy_ReturnsOk()
        {
            ...
            // Assert
            _cachedDbConnector.Verify(c => c.SetDataBaseHierarchy(dbRef, hierarchyToUpdate), Times.Once);
        ...
        public void UpdateHierarchy_InvalidOperation_ReturnsBadRequest()
        {
            ...
            _cachedDbConnector.Setup(x => x.GetDataBaseReference(databaseId)).Returns(dbRef);
            _cachedDbConnector
                .Setup(x => x.SetDataBaseHierarchy(dbRef, hierarchyToUpdate))
        ...
        // Helper delegate for testing TryGetDataBaseHierarchy
